...
<jsp:include page="../board/header.jsp" />
		<form name="joinform" action="updateProcess" method="post" >
			...
			value="${memberinfo.id}" required readOnly>
					
			<b>비밀번호</b>
			<input type="password"  name = "password" placeholder="Enter password"
			value="${memberinfo.getPassword().substring(0,4)}" required readOnly> 
			
			<b>이름</b>
			<input type="text" name="name" 
			value="${memberinfo.name}" placeholder="Enter name">
			
			<b>나이</b>
			<input type="text" name="age"
			 value ="${memberinfo.age}" maxLength="2"  
			...
			if (pattern.test(filename)){
				...
				1.readAsDataURL()
				 ...
				 -Base64 인코딩은 바이너리 데이터를 Text로 변경하는 Encoding으로
				  이를 사용하면 웹 브라우저에서 이미지 동의 파일을 미리보기 기능을 제공할 수 있습니다.
