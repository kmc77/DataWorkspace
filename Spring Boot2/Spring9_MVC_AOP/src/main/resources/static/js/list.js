            ...
            if (data.listcount > 0) { // 총 갯수가 0보다 큰 경우
                ...
                $(data.boardlist).each(function(index, item) {
                    ...
                    if (item.board_RE_LEV > 0) {
                        img = "<img src='../resoutrces/image/AnswerLine.gif'>";
                    }
                    
                    let subject = item.board_SUBJECT.REPLACE(/</g, '&lt;');
                    subject=suject.replace(/>/g, '&gt;')
                    ...
                    output += ' <a href="detail.bo?num=' + item.board_NUM + '">'
                    output += subject.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</a>[' + item.cnt + ']</div></td>'
