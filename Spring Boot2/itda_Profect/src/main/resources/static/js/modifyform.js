$(document).ready(function(){
	...
	// 파라미터 'check'이라는 이름으로 form에 추가하여 전송합니다.
	if (check == 0) {
		const value = $('#filevalue').text();
		const html =  "<input type='hidden' name='check' value='"  + value + "'>";
		console.log(html);
	 	// form
		$("form[name=boardform]").append(html);
	}
    ...
    $(".remove").click(function(){
        ...
        $('#upfile').val(''); // 만약 파일을 선택하고 remove이미지를 클릭하면 <input type=file>의 값도 빈 문자열로 만들어요
    });
