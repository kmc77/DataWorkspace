...
<body>
	...
	<form name="joinform" action="update" method="post" enctype="multipart/form-data">
		<h1>회원 정보 수정</h1>
		<hr>
		<b>아이디</b>
		...
		<span id="email_message"></span> <b>프로필 사진</b> <label> <img src="image/attach.png" width="10px"> 
		<span id="filename">${memberinfo.memberfile}</span> <span id="showImage"> 
				<c:if test='${empty memberinfo.memberfile}'>
					<c:set var='src' value='image/profile.png' />
				</c:if> <c:if test='${!empty memberinfo.memberfile}'>
				...
				</c:if> <img src="${src}" width="20px" alt="profile">
		</span> 
		<%-- accept 속성은 사용자가 파일 선택 대화 상자를 열 때 허용되는 파일의 타입을 지정하는 데 사용됩니다. 
          ...
          <input type="file" accept="파일 확장자|audio/*|video/*|image/*">
             (1) 파일 확장자는 .png , .jpg , .hwp 처럼 (.)으로 시작하는 파일 확장자를 의미합니다.
	...
	$('input[type=file]').change(function(event) {
		...
		if (pattern.test(filename)) {
			...
			/*
			1. readAsDataURL()
				1) readAsDataURL() : FileReader 객체의 메서드로, 선택한 파일을 데이터 URL로 읽도록 요청
				2) DataURL형식으로 읽어온 결과는 reader객체의 result 속성에 저장됩니다.
				3) DataURL형식은 접두사 data:가 붙은 URL이며 바이너리 파일을 Base64로 인코딩하여 ASCII문자열 형식으로
					변환홥니다.
				4) Base64 인코딩은 바이너리 데이터를 Text로 변경하는 Encoding으로
				이를 사용하면 웹 브라우저에서 이미 등의 파일을 미리보기 기능을 제공할 수 있습니다.
			
				2. event.target.files[0] : 파일 리스트에서 첫번째 객체를 가져옵니다.
			*/
			reader.readAsDataURL(event.target.files[0]);
				
				reader.onload = function() {//읽기에 성공했을 때 실헹되는 이벤트 핸들러
					$('#showImage > img').attr('src', this.result);
					
				};
		}else{
			alert('이미지파일(gif,jpg,jpeg,png)이 아닌 경우는 무시됩니다.');
			$(this).val('');
		}
