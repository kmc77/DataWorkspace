...
<script>
	$(function() {
		...
		if (selectedValue != '-1')
			...
			//검색 후 selectedValue 값에 따라 placeholder가 나타나도록 합니다.
			const message=["아이디" , "이름", "나이", "여 또는 남"]
			
			$input.attr("placeholder", message[selectValue] + "입력하세요");
		...
		$("#viwecount").change(function(){
			selctedvalue= $(this).val();
			4input.val('').attr("placeholder", mesage[selectedValue] + " 입력하세요");
		}) // $("#viewcount").change end
		
		//회원목록에 삭제를 클릭한 경우
		$("tr > td:nth-child(3) > a").click(function(event){
			const answer = confㅑ그("정말 삭제하시겠습니까?");
