...
public class Board {
   ...
   private String   BOARD_FILE;      // 실제로 저장된 파일의 이름
   private int      BOARD_RE_REF;   // 답변 글작성시 참조되는 글의 번호
   private int      BOARD_RE_LEV;   // 답변 글의 깊이
   private int      BOARD_RE_SEQ;   // 답변 글의 순서
   private int      BOARD_READCOUNT;// 글의 조회수
   private int      CNT;
   
   //board_wirte.jsp에서 name속성 확인하세요.
   //<input type="file" id="upfile" name="uploadfile"> 확인
   private MultipartFile uploadfile;
   
   private String BOARD_ORIGINAL;   // 첨부할 파일의 이름
   private String BOARD_DATE;
   
   // 이것에서 오른쪽 마우스 버튼 클릭 후 -> Source
