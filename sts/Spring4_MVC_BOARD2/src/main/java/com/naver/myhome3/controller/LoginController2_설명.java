...
public class LoginController2_설명 {
	...
	public String board_write (BbsBean bbs) {
		...
		/*
		 ...
		 즉, list2.jsp에서 
		 commnad 객체의 클래스 이름(첫 글자를 소문자로 바꿉니다.)과 동일한 속성 이름을 사용해서 
		 command 객체를 뷰에 전달합니다.
		 예로 command 객체의 클래스 이름이 BbsBean인 경우 JSP 코드에서는 bbsBean이라는 사용해서 
		 command 객체에 접근할 수 있게 됩니다.
		 
		 
		 반환형이 String형인 경우 View이름을 리턴합니다.
		 */
		return "login/list2"; //webapp/WEB-INF/views/login/list2.jsp를 의미
